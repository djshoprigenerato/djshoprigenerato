...
router.post('/create-checkout-session', express.json(), async (req,res) => {
  try {
    ...
    // cart: [{id, title, price_cents, qty, image_url}]
    let total_cents = 0;
...
router.post('/stripe-webhook', express.raw({ type: 'application/json' }), async (req,res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Fetch line items for this session (optional details for order_items)
    try {
      ...
      console.error('Error persisting session.completed', e);
    }
  }
