// lib/storage.js — upload/rimozione file su Supabase + watermark (file e URL)
...
export async function removePublicUrl(publicUrl) {
  const rel = storagePathFromPublicUrl(publicUrl);
  if (!rel) return;
  await supabase.storage.from(BUCKET).remove([rel]); // ignora se non esiste
}

export async function removeManyPublicUrls(urls = []) {
  const list = [];
  for (const u of urls) {
    const rel = storagePathFromPublicUrl(u);
