...
export default function Checkout(){
  ...
  const applyDiscount = async () => {
    ...
    } catch{ alert('Codice sconto non valido'); setDiscount(null) }
  }

  const pay = async () => {
    try {
      const cartPayload = items.map(i => ({
        id: i.id, title: i.title, qty: i.qty, price_cents: i.price_cents, image_url: i.product_images?.[0]?.url || ''
      }))
      const res = await axios.post('/api/create-checkout-session', { cart: cartPayload, customer, discount })
      window.location.href = res.data.url
    } catch { alert('Errore nel creare la sessione di pagamento') }
  }
    ...
    <div className="container">
      ...
      <div className="card" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16}}>
        <div>
          ...
          {discount && (
            <p className="badge">Applicato: {discount.code} {discount.percent_off?`(-${discount.percent_off}%)`:''}{discount.amount_off_cents?`(-${(discount.amount_off_cents/100).toFixed(2)}€)`:''}</p>
          )}
        </div>
        <div>
          <h3>Riepilogo</h3>
